...
test.describe('Task Management', () => {
  ...
  test('can toggle task completion via API', async ({ page }) => {
    // Create a task directly via API
    const createRes = await page.request.post('http://localhost:3000/v1/projects/api-gw-v2/todos', {
      headers: { 'Content-Type': 'application/json' },
      data: { title: 'API Toggle Test', status: 'todo', priority: 'p2', category: 'Backend' },
    });
    expect(createRes.ok()).toBeTruthy();
    const todo = await createRes.json();
    expect(todo.status).toBe('todo');

    // Toggle via API PATCH
    const patchRes = await page.request.patch(`http://localhost:3000/v1/todos/${todo.id}`, {
      headers: { 'Content-Type': 'application/json' },
      data: { status: 'done' },
    });
    ...
    // Verify it shows as done in the UI
    await page.goto('/');
    await page.locator('#tab-todo').click();
    await page.waitForSelector('.todo-table-header');
    await expect(
      page.locator('.todo-full-row').filter({ hasText: 'API Toggle Test' }).locator('.status-pill')
    ).toHaveClass(/st-done/);
  });
});
